"""
Lucky Wheel System untuk Bot Telegram LootDungeon
Sistem lucky wheel dengan berbagai hadiah dan tingkat kesulitan
...
# Convert datetime string ke datetime object
        ...
        if os.path.exists(self.data_file):
            try:
                with open(self.data_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                # Convert cooldown strings back to datetime objects
                for user_id, user_info in data.items():
                    if 'cooldowns' in user_info:
                        for action, cooldown_str in user_info['cooldowns'].items():
                            user_info['cooldowns'][action] = parse_datetime(cooldown_str)
                return data
    ...
    def _save_user_data(self):
        ...
        try:
            with open(self.data_file, 'w', encoding='utf-8') as f:
                json.dump(self.user_data, f, indent=2, ensure_ascii=False, cls=DateTimeEncoder)
