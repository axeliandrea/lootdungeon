...
# Logging
logging.basicConfig(level=logging.DEBUG)  # SUPER DEBUG MODE
...
async def button_handler(update: Update, context):
    ...
    await q.answer()
    uid = q.from_user.id
    logger.debug(f"Button pressed: {q.data} by {uid}")
    ...
    try:
        if q.data == "lucky":
            ...
            kb = [[InlineKeyboardButton("🎡 BUKA LUCKY WHEEL", web_app=WebAppInfo(url=url))]]
            await q.edit_message_text("Klik tombol di bawah untuk spin!", reply_markup=InlineKeyboardMarkup(kb))
...
async def lucky_wheel_receiver(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.debug(f"Received WebAppData from {update.effective_user.id}")
    try:
        data = json.loads(update.message.web_app_data.data)
        logger.debug(f"WebAppData: {data}")
        ...
        await update.message.reply_text(f"🎉 Kamu mendapat **{pvalue} {ptype.replace('_', ' ').title()}**!")
...
def main():
    try:
        logger.info("Starting bot in SUPER DEBUG mode...")
